...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'vote': return <VotingTerminal user={currentUser} onVoteComplete={handleLogout} />;
      case 'candidates': return <CandidateEnrollment />;
      case 'users': return <UserManagement />;
      case 'config': return <ElectionConfig />;
      case 'logs': return <AuditLogViewer />;
      case 'showcase': return <CandidateShowcase />;
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-slate-900 text-white transition-all duration-300 flex flex-col z-50 fixed md:relative h-full ${isMobile && !isSidebarOpen ? '-translate-x-full' : 'translate-x-0'}`}>
        ...
        <nav className="flex-1 mt-6 px-3 space-y-1">
          {navItems.filter(item => item.roles.includes(currentUser.role)).map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-3 rounded-lg transition-colors ${activeTab === item.id ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
            >
              <item.icon size={20} />
        ...
        <div className="p-4 border-t border-slate-800">
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-3 rounded-lg text-slate-400 hover:bg-red-900/20 hover:text-red-400 transition-colors">
            <LogOut size={20} />
      ...
      <main className="flex-1 overflow-y-auto w-full">
        <header className="bg-white border-b border-slate-200 px-8 py-4 sticky top-0 z-40 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="w-10 h-10 rounded-full bg-slate-200 border-2 border-white shadow-sm flex items-center justify-center text-slate-600 font-bold uppercase">
              {currentUser.name[0]}
            </div>
          </div>
        </header>
